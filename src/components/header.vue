<template>
	<div>
		 <header class="header">
	          ...
	          <div class="navbar-right-container" style="display: flex;">
	            <div class="navbar-menu-container">
	              ...
	              <div class="navbar-cart-container">
	                ...
	                <router-link to="/Cart" class="navbar-link navbar-cart-link" tag="a">
	        ...
	        <div class="md-modal modal-msg md-modal-transition " :class="{'md-show':showLogin}" >
		        <div class="md-modal-inner">
		          <div class="md-top">
		            <div class="md-title" >Login in</div>
		            <button class="md-close" @click="showLogin=false">Close</button>
		          </div>
		            ...
		            <div class="confirm-tips">
		              ...
		              <ul>
		                <li class="regi_form_input">
		                  ...
		                  <input type="text" name="loginname" v-model="usernNme" class="regi_login_input regi_login_input_left" placeholder="User Name">
		                </li>
		                <li class="regi_form_input noMargin">
		                  <i class="icon IconPwd"></i>
		                  <input type="password" name="password" v-model="userPass" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="Password" @keyup.enter="login"><!-- @keyup.enter="login"实现回车登录 -->
		                </li>
		              </ul>
		            </div>
		      ...
		      <div class="md-overlay"  v-if="showLogin"></div>
	      </header>
...
export default{
	...
	methods:{
		login(){
			 if(!this.usernNme || !this.userPass){
		        this.errorTip = true;
		        return
		      }
			axios.get('/static/mock/goods.json',{
					...
					upass:this.userPass
					 ...
					 this.$store.commit("updateUserInfo",this.usernNme);
					console.log(this.usernNme)
		...
		loginKed(){
			//判断是否登陆了开始请求ajax看看是否登陆状态
			axios.get('/static/mock/goods.json')
			.then((res)=>{
				//防灾if里面，此处测试
				this.$store.commit("updateUserInfo",'1');
				if(res.data==0){
					this.showName=true
				}
		 ...
		 getCartCount(){  // 查询购物车商品数量
	      axios.get("/static/mock/goods.json").then(res=>{
	      	//个数
	        var res = res.data;
	        this.$store.commit("initCartCount",3);
	      });
	    }
	}
}
