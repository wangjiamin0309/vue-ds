            ...
            <ul class="cart-item-list">
              <li v-for="item in cartList">
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{item.prodcutnumber*item.prodcutPrice | currencys('$')}}</div>
                </div>
              </li>
            </ul>
          </div>
          ...
          <div class="price-count">
            <ul>
              <li>
                ...
                <span>{{totalPrice | currencys('$')}}</span>
              </li>
              <li>
                <span>Shipping:</span>
                <span>{{shipping | currencys('$')}}</span>
              </li>
              <li>
                <span>Discount:</span>
                <span>{{discount | currencys('$')}}</span>
              </li>
              <li>
                <span>Tax:</span>
                <span>{{tax | currencys('$')}}</span>
              </li>
              <li class="order-total-price">
                <span>Order total:</span>
                <span>{{orderTotal | currencys('$')}}</span>
              </li>
            </ul>
          </div>
        </div>
    ...
    methods:{   	
	    init(){
	    		axios.get('/static/mock/goods.json').then((res)=>{
		    		//console.log(res)
		    		 this.cartList = res.data.result.slice(0,2);
		    		 this.cartList.forEach((item)=>{
		    		 	this.totalPrice+=item.prodcutnumber*item.prodcutPrice
		    		 })
		    		  this.orderTotal = this.totalPrice+this.shipping-this.discount+this.tax;  // 获取用户最终支付的金额，(总金额+配送费-折扣+税费)
		    	},(err)=>{
	    ...
	    payMent(){
	    	 ...
	    	 var chuan=this.orderTotal;
	    	 //console.log(addressId)
	    	 this.$router.push({path:'/OrderSuccess',query:{total:chuan}})
	    }
    },
    mounted(){
    	this.init()
