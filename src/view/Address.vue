<template>
    <div>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in AddressListFilter" :class="{'check':checkedIndex==index}" v-if="item.map" @click="checkedIndex=index;selectedAddrId=item.productId"><!--此处是地址的id-->
                    <dl>
                      <dt>Jack</dt>
                      <dd class="address">{{item.map}}</dd>
                      <dd class="tel">{{item.iphone}}</dd>
                    </dl>
                    <div class="addr-opration addr-del" @click="detalMap(item.productId)"><!--此处是地址的id-->
                      <a href="javascript:;" class="addr-del-btn">
                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                      </a>
                    </div>
                   																		
                    <div class="addr-opration addr-set-default" @click="setMap(item.productId)"> <!--此处是地址的id-->
                      <a href="javascript:;" class="addr-set-default-btn" v-if="!item.defaultMap"><i>Set default</i></a>
                    </div>
                    <div class="addr-opration addr-default" v-if="item.defaultMap">Default address</div>
                  </li>
      ...
      <Modal :mdShows="showMapModel" @closeMD="closeModel">
      		<p slot="message">确定删除此地址</p>
	      	<div slot="btnGroup">
	      		<a class="btn" @click="detalMapSure()">确定</a>
	      		<a class="btn" @click="closeModel">关闭</a>
	      	</div>
      </Modal>
     <Footer></Footer>
    </div>
